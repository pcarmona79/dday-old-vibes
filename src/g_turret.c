 ...
 ************************************
 *   $Source: /var/lib/cvs/dday42sk/src/g_turret.c,v $
 *   $Revision: 1.1.1.1 $
 *   $Date: 2003/10/06 22:57:01 $
 * 
 ***********************************
...
void fire_shell_t (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	edict_t	*shell, *trail;
...
//	shell->s.effects |= EF_GRENADE;
	shell->nextthink = shell->nextthink = level.time + 8000.0/speed;
...
//	gi.positioned_sound (start, self, CHAN_WEAPON, gi.soundindex(self->obj_name), 1, ATTN_NORM, 0);
	...
	if (!strcmp(self->killtarget, "shell")) //ammo_type
	{
		...
		if (self->numfired)
			...
			speed = 1000;
		fire_shell_t (self->teammaster->owner, start, f, damage, speed, 150, damage);
	}
