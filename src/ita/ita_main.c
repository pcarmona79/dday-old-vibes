...
void InitFunctions(void)
{
	...
	G_FreeEdict = (void (*)(edict_t *ed)) PlayerFindFunction("G_FreeEdict");
	...
	T_Damage = (void (*)(edict_t *, edict_t *, edict_t *, vec3_t, vec3_t, vec3_t, int, int, int, int))
			PlayerFindFunction("T_Damage");
	T_RadiusDamage = (void (*)(edict_t *, edict_t *, float, edict_t *, float, int)) PlayerFindFunction("T_RadiusDamage");
	Play_Ricochet_Noise = (void (*)(edict_t *, vec3_t)) PlayerFindFunction("Play_Ricochet_Noise");
	Smoke_Effect = (void (*)(vec3_t, float)) PlayerFindFunction("Smoke_Effect");
	ThrowDebris = (void (*)(edict_t *, char *, float, vec3_t)) PlayerFindFunction("ThrowDebris");
...
{
	ptrgi->imageindex("scope_ita");
	ptrgi->imageindex("victory_ita");

	ptrgi->soundindex("ita/victory.wav");

	ptrgi->modelindex("players/ita/tris.md2");
	...
	// ITA 
	ptrgi->modelindex("players/ita/w_b34.md2");
	ptrgi->modelindex("players/ita/w_carcano.md2");
	ptrgi->modelindex("players/ita/w_b38.md2");
	ptrgi->modelindex("players/ita/w_k43.md2");
	ptrgi->modelindex("players/ita/w_b3842.md2");
	ptrgi->modelindex("players/ita/w_panzerf.md2");
	ptrgi->modelindex("players/ita/w_panzerfmt.md2");
	ptrgi->modelindex("players/ita/w_m98ks.md2");
	ptrgi->modelindex("players/ita/a_masher.md2");
	ptrgi->modelindex("players/ita/w_breda.md2");

	ptrgi->modelindex("models/objects/panzrocket/tris.md2");

	//faf:  airstrike plane
	ptrgi->modelindex("models/ships/itaplane/tris.md2");
...
static userdll_export_t userdll_export =
{
	...
	ITA_MOS_List,
	"ita",					//team id
	"ita"					//this is the player model to use when spawning
};
...
userdll_export_t ITAGetAPI(userdll_import_t udit)
{
