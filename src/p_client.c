 ...
 ************************************
 *   $Source: /var/lib/cvs/dday42sk/src/p_client.c,v $
 *   $Revision: 1.2 $
			...
			if (message)
			{
				...
				if (deathmatch->value)
				{
					if (ff) {
						...
						//kernel: teamkills check
						if (team_kill->value && teamkills_check->value) {

							attacker->client->resp.teamkills++;
							if (attacker->client->resp.teamkills == 1 ||
							    attacker->client->resp.first_teamkill + teamkills_time->value < level.time) {
								// reset teamkill counter
								attacker->client->resp.first_teamkill = level.time;
								attacker->client->resp.teamkills = 1;
							}
							if (attacker->client->resp.first_teamkill + teamkills_time->value >= level.time &&
								 attacker->client->resp.teamkills > teamkills_max->value) {
								// team killer!! grr!!
								gi.bprintf (PRINT_HIGH, "%s is a bastard team killer!\n", attacker->client->pers.netname);
								/*gi.centerprintf (attacker, "YOU ARE A BASTARD TEAM KILLER!!\n\nFirst: %.2f Level: %.2f\nDiff: %.2f TK Time: %.2f",
										 attacker->client->resp.first_teamkill, level.time,
										 level.time - attacker->client->resp.first_teamkill,
										 teamkills_time->value);*/
								gi.centerprintf (attacker, "YOU ARE A BASTARD TEAM KILLER!!");
								attacker->client->resp.teamkills = 0;
								attacker->client->resp.first_teamkill = 0.0;
								DropClient (attacker);
							}
							else {
								/*gi.centerprintf (attacker, "Hey! You've killed %d of your teammates!\n\nFirst: %.2f Level: %.2f\nDiff: %.2f TK Time: %.2f",
										 attacker->client->resp.teamkills,
										 attacker->client->resp.first_teamkill, level.time,
										 level.time - attacker->client->resp.first_teamkill,
										 teamkills_time->value);*/
...
//faf	if (level.framenum > ((int)level_wait->value * 10))
	...
	//kernel: teamkills count
	ent->client->resp.teamkills = 0;
	ent->client->resp.first_teamkill = 0.0;
...
#ifdef DEBUG
		gi.cprintf (ent, PRINT_HIGH, "D-Day: Normandy [DEVELOPMENT BUILD]\n"
                                     "DATE:     %s\n"
									 "VERSION:  %s\n", __DATE__, DEVVERSION);
#endif
...
Added by Kernel
This will be called to kick a teamkiller
==============
*/
void DropClient (edict_t *ent)
{
	gi.bprintf (PRINT_HIGH, "%s was kicked\n", ent->client->pers.netname);
	gi.cprintf (ent, PRINT_HIGH, "You were kicked from the game\n");
	gi.cprintf (ent, PRINT_HIGH, "%s disconnected\n", ent->client->pers.netname);
	gi.WriteByte (7); // send disconnect message
	gi.unicast (ent, true);
}
