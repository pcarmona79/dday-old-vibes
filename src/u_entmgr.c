 ...
 ************************************
 *   $Source: /var/lib/cvs/dday42sk/src/u_entmgr.c,v $
 *   $Revision: 1.1.1.1 $
 *   $Date: 2003/10/06 22:56:18 $
 *
 ***********************************
...
gitem_t *InsertItem(gitem_t *it, spawn_t *spawnInfo)
{
	...
	if (!spot && i < (MAX_ITEMS - 1))
	{
		// kernel: adds 1 to the last index to avoid overwrite the last element
		inc_items = i + 1;
